...
const MOVEMENT_REPORT_TYPES = [
  { value: 'STOCK_MOVEMENT', label: 'Stock Movement History', icon: Activity },
  { value: 'TRANSFER_REPORT', label: 'Transfer Report', icon: ArrowRightLeft },
  { value: 'ITEM_ENTRY_REPORT', label: 'Item Entry Report', icon: TrendingUp },
  { value: 'WITHDRAWAL_REPORT', label: 'Withdrawal Report', icon: TrendingDown },
]

export function MovementReports({ warehouses }: MovementReportsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {MOVEMENT_REPORT_TYPES.map((reportType) => {
            ...
            <Card key={reportType.value} className="hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                ...
                <p className="text-gray-600 text-xs mb-4">
                  {reportType.value === 'STOCK_MOVEMENT' && 'Complete audit trail of all movements'}
                  {reportType.value === 'TRANSFER_REPORT' && 'Inter-warehouse transfer tracking'}
                  {reportType.value === 'ITEM_ENTRY_REPORT' && 'All item entries with landed costs'}
                  {reportType.value === 'WITHDRAWAL_REPORT' && 'Material withdrawals and issuances'}
                </p>
